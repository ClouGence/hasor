...
SolrDate = function(date) {
	// ["Sat Mar 03 11:00:00 CET 2012", "Sat", "Mar", "03", "11:00:00", "CET",
	// "2012"]
...
var sammy = $.sammy(function() {
	...
	this.bind('error', function(message, original_error) {
	...
	this.before({}, function(context) {
		...
		$('li[id].active', menu_wrapper).removeClass('active');
		$('li.active', menu_wrapper).removeClass('active');
		// global dashboard doesn't have params.splat
		if (!this.params.splat) {
			this.params.splat = [ '~index' ];
		}
		var selector = '~' === this.params.splat[0][0] ? '#' + this.params.splat[0].replace(/^~/, '') + '.global' : '#core-selector #' + this.params.splat[0].replace(/\./g, '__');
		...
		if (active_element.hasClass('global')) {
			...
			if (this.params.splat[1]) {
				$('.' + this.params.splat[1], active_element).addClass('active');
			}

			$('#core-selector option[selected]').removeAttr('selected').trigger('liszt:updated');
			$('#core-selector .chzn-container > a').addClass('chzn-default');
		} else {
			active_element.attr('selected', 'selected').trigger('liszt:updated');

			if (!this.params.splat[1]) {
				this.params.splat[1] = 'overview';
			}
			$('#core-menu .' + this.params.splat[1]).addClass('active');
...
var solr_admin = function(app_config) {
	that = this;
	menu_element = null;
	is_multicore = null, cores_data = null, active_core = null;
	config = app_config, params = null, dashboard_values = null, schema_browser_data = null, plugin_data = null;
	...
	show_global_error = function(error) {
		var main = $('#main');
		$('div[id$="-wrapper"]', main).remove();
		main.addClass('error').append(error);
	...
	this.set_cores_data = function set_cores_data(cores) {
		...
		for ( var core_name in that.cores_data) {
			...
			var core_tpl = '<option id="' + core_name.replace(/\./g, '__') + '" '//
					+ 'class="' + classes.join(' ') + '" '//
		...
		if (has_cores) {
			that.menu_element.append(core_list.join("\n")).trigger('liszt:updated');
		}
		...
		if (has_cores) {
			...
			cores_element.find('.chzn-container').css('width', selector_width + 'px');
			cores_element.find('.chzn-drop').css('width', (selector_width - 2) + 'px');
		}
		this.check_for_init_failures(cores);
	};

	this.remove_init_failures = function remove_init_failures() {
		$('#init-failures').hide().find('ul').empty();
	}
	...
	this.check_for_init_failures = function check_for_init_failures(cores) {
		...
		for ( var core_name in cores.initFailures) {
			failures.push('<li><strong>' + core_name.esc() + ':</strong>' + cores.initFailures[core_name].esc() + '</li>');
		}
		...
		$('#init-failures').show().find('ul').html(failures.join("\n"));
	}
	...
	this.run = function() {
		...
		var ajaxSuccess = function(response) {
			...
			that.menu_element.chosen().off('change').on('change', function(event) {
				location.href = $('option:selected', this).val();
				return false;
			}).on('liszt:updated', function(event) {
				var core_name = $('option:selected', this).text();
				var insertHTML = // Keep this in alphabetical order after the overview
				...
				'<li class="plugins"><a href="#/' + core_name + '/plugins"><span>插件</span></a></li>' + //
				'<li class="replication"><a href="#/' + core_name + '/replication"><span>Replication</span></a></li>' + //
				'<li class="ping"><a rel="' + that.config.solr_path + '/' + core_name + '/admin/ping"><span>Ping</span></a></li>';//
				that.core_menu.html(insertHTML).show();
				$(that.core_menu).show();
				if (!core_name) {
					that.core_menu.hide().empty();
				}
			});
			...
			$.ajax({
				...
				success : function(response) {
					...
					if (environment_args) {
						wrapper.addClass('has-environment');
						if (environment_args[1]) {
							environment_element.html(environment_args[1]);
						}
						if (environment_args[2]) {
							environment_element.addClass(environment_args[2]);
						}
					} else {
						wrapper.removeClass('has-environment');
					}
					// cloud
					var cloud_nav_element = $('#menu #cloud');
					if (cloud_args) {
						cloud_nav_element.show();
					}
					// sammy
					sammy.run(location.hash);
				},
				error : function() {
					show_global_error('<div class="message"><p>Unable to load environment info from <code>'//
							+ system_url.esc()//
							+ '</code>.</p>'//
		...
		$.ajax({
			...
			dataType : 'json',
			beforeSend : function(arr, form, options) {
				$('#content').html('<div id="index"><div class="loader">加载中 ...</div></div>');
			},
			success : function(response) {
	...
	this.format_number = function format_number(number) {
		...
		return (number || 0).toString().replace(/\B(?=(\d{3})+(?!\d))/g, sep[browser.locale] || sep[browser.language] || sep['_']);
	};
};
