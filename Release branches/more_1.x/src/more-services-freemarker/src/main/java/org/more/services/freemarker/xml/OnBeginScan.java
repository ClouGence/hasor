...
import org.more.services.freemarker.Tag;
/**
 * 该类负责注册注解扫描器。
 * @version : 2011-4-22
...
class OnBeginScan implements EventListener<BeginScanEvent> {
    ...
    /**该类负责注册注解扫描器。*/
    public OnBeginScan(FreemarkerService service) {
    ...
    public void onEvent(BeginScanEvent event, Sequence sequence) throws Throwable {
        ...
        log.debug("regedit FreeMarker Anno Watch to FreeMarkerService.");
        params.annoService.registerAnnoKeepWatch(Tag.class, new Watch_Tag(this.service));
        params.annoService.registerAnnoKeepWatch(Function.class, new Watch_Function(this.service));
    };
};