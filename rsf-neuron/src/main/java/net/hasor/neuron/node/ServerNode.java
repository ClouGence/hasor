...
public class ServerNode implements EventListener<Object>, AskNameService {
    ...
    public void init() throws UnknownHostException, URISyntaxException {
        // .基础属性初始化
        if (StringUtils.equalsIgnoreCase(this.serverID, "local")) {
            this.serverID = NetworkUtils.finalBindAddress(this.serverID).getHostAddress();
        }
        this.currentTerm = "0";
        this.votedFor = null;
        this.status = ServerStatus.Follower;
        this.rsfContext.getEnvironment().getEventContext().addListener(NeuronEvent.ServerStatus, this);
        // .集群信息
        String serverSetting = this.rsfContext.getEnvironment().getSettings().getString("hasor.rsfNeuron.servers");
        String[] serverArrays = null;
        if (StringUtils.isEmpty(serverSetting)) {
            serverArrays = new String[0];
        } else {
            serverArrays = serverSetting.split(",");
        }
        // .添加节点
        this.clusterManager.addNode(this.rsfContext.bindAddress());
