 ...
 */
public class GuiceRegisterFactoryCreater implements RegisterFactoryCreater {
    public RegisterFactory create(Environment env) {
        return new GuiceRegisterFactory();
    }
}
/*RegisterManager接口实现*/
class GuiceRegisterFactory extends AbstractRegisterFactory {
    ...
    public <T> T getInstance(RegisterInfo<T> oriType) {
        ...
        return this.guiceInjector.getInstance(oriType.getType());
    }
    //
    /*-------------------------------------------------------------------------------add to Guice*/
    public void doInitializeCompleted(AbstractAppContext appContext) {
        super.doInitializeCompleted(appContext);
        this.guiceInjector = Guice.createInjector(new Module() {
