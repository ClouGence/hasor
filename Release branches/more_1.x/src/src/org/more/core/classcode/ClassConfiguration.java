...
class ClassConfiguration {
    ...
    private ArrayList<String> renderAopMethodList      = null;
    //
    public ClassConfiguration(ClassBuilder classBuilder, BuilderClassAdapter builderAdapter, AopClassAdapter aopAdapter) {
            ...
            for (int i = 0; i < this.renderAopMethodList.size(); i++) {
                ...
                for (Method m : methodArrays) {
                    ...
                    if (index != -1) {
                        ...
                        //
                        AopBeforeListener[] _aopBeforeListener = aopStrategy.filterAopBeforeListener(aopBeforeListener);
                        AopReturningListener[] _aopReturningListener = aopStrategy.filterAopReturningListener(aopReturningListener);
                        AopThrowingListener[] _aopThrowingListener = aopStrategy.filterAopThrowingListener(aopThrowingListener);
                        AopInvokeFilter[] _aopInvokeFilter = aopStrategy.filterAopInvokeFilter(aopInvokeFilter);
                        //
