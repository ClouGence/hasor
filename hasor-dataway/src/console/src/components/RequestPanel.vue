  ...
  <div class="requestPanel">
    <div class="request-btns" style="height: 30px;">
      <el-button-group>
        ...
        <el-tooltip
          v-if="this.panelMode === 'req_parameters'"
          class="item"
          effect="dark"
        ...
        <el-tooltip
          v-if="this.panelMode === 'req_headers'"
          class="item"
          effect="dark"
          placement="bottom-end"
          content="Add Header"
    ...
    <el-tabs v-model="panelMode" class="request-tabs" type="card">
      ...
      <el-tab-pane name="req_headers" label="Headers" lazy>
        <el-table
          ref="requestHeaderTable"
          :data="headerDataCopy"
          :height="headerPanelHeight"
          border
          empty-text="No Header"
        >
          <el-table-column prop="checked" width="24" :resizable="false">
            <template slot="header">
              <el-checkbox
                v-model="headerSelectAllStatus"
                name="type"
                :indeterminate="headerSelectIndeterminateStatus"
                @change="handleHeaderCheckAllChange"
              />
            </template>
            ...
            <template slot-scope="scope">
              <el-tooltip class="item" effect="dark" content="Delete" placement="left">
                <el-button
                  ...
                  icon="el-icon-delete"
                  circle
                  @click.native="handleHeaderDelete(scope.row,scope.$index)"
                />
              </el-tooltip>
            </template>
          </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleParametersFormatter() {
      try {
        this.requestBodyCopy = JSON.stringify(
          JSON.parse(this.requestBodyCopy),
          null,
          2
        )
    ...
    updateIndeterminate() {
      ...
      this.headerSelectIndeterminateStatus =
        checkedCount > 0 && checkedCount !== this.headerDataCopy.length
    },
    // 触发执行
    triggerRun() {
      this.$emit('onRun')
