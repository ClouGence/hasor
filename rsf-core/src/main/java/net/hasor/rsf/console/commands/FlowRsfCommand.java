...
public class FlowRsfCommand implements RsfCommand {
    ...
    public String helpInfo() {
        return "service flowControl show/update.\r\n"//
                + " - flow             (show help info.)\r\n"// 
                + " - flow -s default  (show default service flowControl info.)\r\n"//
                + " - rule -s XXXX     (show service flowControl info of XXXX.)\r\n"//
                //
                + " - flow -u default  (update default service flowControl info .)\r\n"//
                + " - flow -u XXXX     (update service flowControl info of XXXX.)\r\n"//
                //
                + " - flow -c default  (clean default service flowControl info .)\r\n"//
    ...
    private void showFlowControl(StringWriter sw, String nameArg, RsfContext rsfContext) throws IOException {
        ...
        if ("default".equalsIgnoreCase(nameArg)) {
            body = rsfContext.getUpdater().defaultFlowControl();
        } else {
            body = rsfContext.getUpdater().flowControl(nameArg);
        }
        //2.write
    ...
    private void updateFlowControl(StringWriter sw, String nameArg, RsfCommandRequest request) {
        ...
        String scriptBody = request.getRequestBody();
        if ("default".equalsIgnoreCase(nameArg)) {
            boolean result = rsfContext.getUpdater().updateDefaultFlowControl(scriptBody);
            sw.write("[" + (result ? "SUCCEED" : "FAILED") + "] update default FlowControl.");
            return;
        } else {
    ...
    private void cleanFlowControl(StringWriter sw, String nameArg, RsfCommandRequest request) {
        RsfContext rsfContext = request.getRsfContext();
        if ("default".equalsIgnoreCase(nameArg)) {
            boolean result = rsfContext.getUpdater().updateDefaultFlowControl(null);
            sw.write("[" + (result ? "SUCCEED" : "FAILED") + "] clean default FlowControl.");
            return;
        } else {
            if (rsfContext.getServiceInfo(nameArg) == null) {
                sw.write("[ERROR] serviceID is not exist.");
