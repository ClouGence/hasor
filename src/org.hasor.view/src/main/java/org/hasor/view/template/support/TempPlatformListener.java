...
public class TempPlatformListener extends WebHasorModule {
    private TempSettings freemarkerSettings = null;
    private FreemarkerManager  freemarkerManager  = null;
    /**初始化.*/
    @Override
    public void init(WebApiBinder apiBinder) {
        this.freemarkerSettings = new TempSettings();
        this.freemarkerSettings.loadConfig(apiBinder.getSettings());
        //
        apiBinder.getGuiceBinder().bind(TempSettings.class).toInstance(freemarkerSettings);
        ...
        if (this.freemarkerSettings.isEnable() == true) {
            ...
            if (suffix != null)
                for (String suf : suffix)
                    event.serve(suf).with(TempHttpServlet.class);
        }
    }
    //
    /***/
