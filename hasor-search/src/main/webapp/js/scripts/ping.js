 ...
 */
$('.ping a', app.core_menu).live('click', function(event) {
	$.ajax({
		...
		success : function(response, text_status, xhr) {
			...
			if (0 === qtime_element.size()) {
				...
				$(this).append(qtime_element);
			}
			$('span', qtime_element).html(response.responseHeader.QTime + 'ms');
		},
		error : function(xhr, text_status, error_thrown) {
			$(this).attr('title', '/admin/ping is not configured (' + xhr.status + ': ' + error_thrown + ')');
