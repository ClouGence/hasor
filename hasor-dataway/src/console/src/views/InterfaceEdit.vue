<template>
  <div>
    <div class="monacoEditorHeader">
      <div style="width: 50%; margin-top: 2px; display: inline-table;">
        <el-tooltip
          ...
          effect="dark"
          placement="bottom"
          :content="apiInfo.comment || defaultComment"
          :disabled="showComment"
        >
          <el-input
            v-model="apiInfo.apiPath"
            placeholder="the path to access this Api"
      ...
      <div v-if="showComment" class="comment">
        <el-input
          v-model="apiInfo.comment"
          placeholder="Api's comment."
          size="mini"
          @input="handleCommentOnchange"
        >
    ...
    <div :style="{height: panelHeight + 'px'}">
      <SplitPane
        :min-percent="10"
        :default-percent="panelPercentVertical"
        split="vertical"
      ...
      >
        ...
        <template slot="paneR">
          <SplitPane
            :min-percent="10"
            :default-percent="panelPercentHorizontal"
            split="horizontal"
...
export default {
  ...
  methods: {
    ...
    layoutMonacoEditor() {
      ...
      const monacoEditorWidth =
        document.documentElement.clientWidth * (this.panelPercentVertical / 100)
      ...
      const widthSize =
        document.documentElement.clientWidth - monacoEditorWidth - 2
      this.$refs.editerRequestPanel.doLayout(heightSize * dataNum, widthSize)
      this.$refs.editerResponsePanel.doLayout(
        heightSize * (1 - dataNum),
        widthSize
      )
    },
    ...
    loadApiDetail() {
      const self = this
      request(
        ApiUrl.apiDetail + '?id=' + self.apiInfo.apiID,
        {
          method: 'GET'
        },
        response => {
    ...
    loadEditorMode() {
      ...
      if (this.apiInfo.codeType.toLowerCase() === 'dataql') {
        ...
        if (
          this.newCode &&
          this.monacoEditor.getValue().trim() ===
            '-- a new Query.\nselect #{message};'
        ) {
