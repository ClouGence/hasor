...
import net.hasor.rsf.common.executes.ExecutesManager;
import net.hasor.rsf.common.executes.NameThreadFactory;
...
public abstract class AbstractRsfContext implements RsfContext {
    ...
    public <T> ServiceMetaData<T> getService(String serviceName, String group, String version) {
        return this.getBindCenter().getService(serviceName, group, version);
    }
    /**获取注册中心。*/
    public AbstractBindCenter getBindCenter() {
        if (this.bindCenter == null)
            this.bindCenter = this.createBindCenter();
        return this.bindCenter;
    }
    public abstract AbstractBindCenter createBindCenter();
}