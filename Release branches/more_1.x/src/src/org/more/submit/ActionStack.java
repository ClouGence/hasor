...
public class ActionStack implements IAttribute, ScopeEnum {
     ...
     * 如果企图获取不存在的作用域则会引发NoDefinitionException异常。
     */
    ...
    /**设置当前操作的作用域，如果企图设置为未定义的作用域则会引发NoDefinitionException异常。注意scope参数可以是ScopeEnum接口的常量定义。*/
    public void setScope(String scope) throws NoDefinitionException {
        if (this.scopeMap.containsKey(scope) == false)
            throw new NoDefinitionException("不能设置到未定义的作用域，这个作用域可能没有注册。");
