...
public class ActionManagerBuilder implements ActionBinder {
    ...
    public void buildManager(Binder binder) {
        for (InternalNameSpaceBindingBuilder item : this.nameSpaceMap.values()) {
            ControllerNameSpace ans = item.buildNameSpace(binder);
            binder.bind(ControllerNameSpace.class).annotatedWith(UniqueAnnotations.create()).toInstance(ans);
        }
    }
    //
    ...
    private static class InternalNameSpaceBindingBuilder implements NameSpaceBindingBuilder {
        ...
        //
        public ControllerNameSpace buildNameSpace(Binder binder) {
            ControllerNameSpace ans = new ControllerNameSpace(this.namespace);
    ...
    private static abstract class AbstractActionBindingBuilder implements ActionBindingBuilder {
        public abstract void buildActionNameSpace(Binder binder, ControllerNameSpace ans);
    }
    //
    /**对一个Action进行定义*/
    private static class ActionBindingBuilderImpl extends AbstractActionBindingBuilder {
        ...
        public void buildActionNameSpace(Binder binder, ControllerNameSpace ans) {
            //1.HttpMethod
