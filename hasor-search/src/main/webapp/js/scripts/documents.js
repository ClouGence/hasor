...
function addWizardFields(active_core, wizard) {
	...
	function(data) {
		...
		$.each(data.fields, function(i, item) {
			...
			if (item.name != "_version_") {
				select_options += '<option name="' + item.name + '">' + item.name + '</option>';
			}
		});
	...
	wizard_doc.die('focusin').live('focusin', function(event) {
	...
	add_field.die("click").live("click", function(event) {
...
sammy.get(new RegExp(app.core_regex_base + '\\/(documents)$'), function(context) {
	var active_core = this.active_core;
	var core_basepath = active_core.attr('data-basepath');
	var content_element = $('#content');

	$.post('tpl/documents.html', function(template) {
		...
		$('#document', documents_form).die('focusin').live('focusin', function(event) {
		...
		doc_type_select.die('change').live('change', function(event) {
			var document_type = $('#document-type', documents_form).val();
			var file_upload = $('#file-upload', documents_form);
			...
			if (document_type == "json") {
				...
				toggles(documents_form, false, false, true, '<doc>\n' + '<field name="id">change.me</field>' + '<field name="title">change.me</field>' + '</doc>', false);
				$("#attribs").show();
			}
			return false;
		});
		...
		documents_form.die('submit').live('submit', function(event) {
			var form_values = [];
			var handler_path = $('#qt', documents_form).val();
			if ('/' !== handler_path[0]) {
				form_values.push({
					name : 'qt',
					value : handler_path.esc()
				});
				handler_path = '/update';
			}

			var document_url = window.location.protocol + '//' + window.location.host + core_basepath + handler_path + '?wt=json';

			url_element.attr('href', document_url).text(document_url).trigger('change');
			...
			if (document_type == "json" || document_type == "wizard") {
				...
				the_command = "{" + '"add":{ "doc":' + the_document + "," + '"boost":' + boost + "," + '"overwrite":' + overwrite + "," + '"commitWithin":' + commit_within + "}}";
				...
				the_command = '<add commitWithin="' + commit_within + '" overwrite="' + overwrite + '"' + ">" + the_document + "</add>";
			...
			if (doingFileUpload == false) {
				$.ajax({
					...
					success : function(response, text_status, xhr) {
						...
						this.html('<div><span class="description">Status</span>: ' + text_status + '</div>' + '<div><span class="description">Response:</span>' + '<pre class="syntax language-json"><code>' + content_generator['json'](xhr.responseText) + "</code></pre></div>");
						result_element.show();
					},
					error : function(xhr, text_status, error_thrown) {
						console.log("error: " + text_status + " thrown: " + error_thrown);
						this.html('<div><span class="description">Status</span>: ' + text_status + '</div><div><span class="description">Error:</span> ' + '' + error_thrown + '</div>' + '<div><span class="description">Error</span>:' + '<pre class="syntax language-json"><code>' + content_generator['json'](xhr.responseText) + '</code></pre></div>');
						result_element.show();
					},
...
/*
 * Sample docs: <doc boost="2.5"> <field name="id">05991</field> <field name="title" boost="2.0">Bridgewater</field> </doc>
 * 
 * {"id":"foo","title":"blah"}
 * 
 */