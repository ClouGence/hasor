...
import net.hasor.plugins.secweb.SecurityForward.ForwardType;
import net.hasor.security.Digest;
import net.hasor.security._.SecurityConfig;
...
public class SecuritySettings implements SecurityConfig {
    ...
    private void readIncludeRules(XmlProperty rulesIncludes) {
        ...
        for (XmlProperty item : includeList) {
            ...
            //
            Hasor.info("read Include Rule : %s", matcher);
        }
    }
    //
    private void readExcludesRules(XmlProperty rulesExcludes) {
        ...
        for (XmlProperty item : excludeList) {
            ...
            //
            Hasor.info("read Exclude Rule : %s", matcher);
        }
    }
    //
        ...
        for (XmlProperty item : dispatcherList) {
            ...
            try {
                ...
                if (Digest.class.isAssignableFrom(digestClass) == false) {
                    Hasor.warning("%s digest %s is not implemented CodeDigest.", digestCode, digestType);
                    continue;
                }
                this.digestMap.put(digestCode, (Class<Digest>) digestClass);
                Hasor.info("read CodeDigest %s : %s", digestCode, digestType);
            } catch (Exception e) {
                Hasor.warning("create %s an error.%s", digestType, e);
            }
        }
    }
    //
