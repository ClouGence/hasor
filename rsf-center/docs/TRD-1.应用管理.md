...
##一、架构设计
###双数据源 
应用管理模块采用双数据源（内存HSQL、持久化MySQL）两个源。

1. HSQL内存数据库的目的是为了加速查询。
2. MySQL数据库的目的是为了持久化（应用、服务接口、路由策略）信息。

###增量同步
1. 所有数据的（`增、删、改`）操作都先对MySQL进行，当操作成功之后再同步操作到HSQL。
2. 如果对HSQL的（`增、删、改`）操作出现失败，则删除HSQL中所有相关的应用数据。然后在重新同步MySQL的应用数据。

###分布式数据同步
    （在二期中完善）

###双源查询
1. 所有对数据库的查询操作都优先走HSQL。
2. 如果HSQL中没有查到相关信息则从MySQL中查询该应用的数据填充到HSQL，然后在从HSQL中查询数据。
