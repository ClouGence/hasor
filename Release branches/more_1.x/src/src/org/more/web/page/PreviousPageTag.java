...
public class PreviousPageTag extends BasePageTag {
    /**  */
    private static final long serialVersionUID = -950249720490688075L;
    ...
    protected int doStartPageTag() throws JspException {
        ...
        //要求在两边时候
        if (first <= -1 && last <= -1)
            if (this.page.info.isFirstIndex(0, item_c) == true)
        ...
        //要求在尾部
        if (last >= 0)
            if (this.page.info.isLastIndex(first, item_c) == true)
                return this.thisDoTag();
        return SKIP_BODY;
    }
