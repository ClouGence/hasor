<template>
  <SplitPane
    :min-percent="30"
    :default-percent="verticalPanelPercent"
    split="vertical"
    @resize="handleVerticalSplitResize"
  >
    <template slot="paneL">
      <el-table
        ref="interfaceTable"
      ...
      >
          ...
          <template slot-scope="scope">
            <el-tooltip class="item" effect="dark" content="Choose to Test" placement="right">
              <el-checkbox
                ...
                name="type"
                @change="handleApiDataChange(scope.row)"
              />
            </el-tooltip>
          </template>
        </el-table-column>
        <el-table-column prop="path" label="Api" :show-overflow-tooltip="true" :resizable="false">
          ...
          <template slot-scope="scope">
            <el-tag
              size="mini"
              style="float: left;width: 45px;text-align: center;margin-right: 2px;"
              effect="dark"
              :type="tableRowMethodTagClassName(scope.row).css"
            >{{ tableRowMethodTagClassName(scope.row).title }}</el-tag>
            <el-tag
              size="mini"
              style="float: left;width: 65px;text-align: center;"
              :type="tableRowStatusTagClassName(scope.row).css"
            >{{ tableRowStatusTagClassName(scope.row).title }}</el-tag>
            <span
              style="overflow-x: hidden;"
            >{{ requestPath(scope.row.path) }}&nbsp;&nbsp;&nbsp;&nbsp;</span>
            <span
              style="color: #adadad;display: contents;float: right; overflow-x: hidden;"
    ...
    <template slot="paneR">
      <split-pane
        :min-percent="30"
        :default-percent="horizontalPanelPercent"
        split="horizontal"
        @resize="handleHorizontalSplitResize"
      >
        <template slot="paneL">
          <RequestPanel
            ref="listRequestPanel"
...
export default {
  ...
  mounted() {
    this.handleSplitResize(
      this.verticalPanelPercent,
      this.horizontalPanelPercent
    )
    //
    const self = this
    this._resize = () => {
      return (() => {
        self.handleSplitResize(
          self.verticalPanelPercent,
          self.horizontalPanelPercent
        )
      })()
    }
  ...
  methods: {
    ...
    handleSplitResize(verticalPercent, horizontalPercent) {
      ...
      //
      this.$refs.listRequestPanel.doLayout(
        heightSize * horizontalDataNum,
        widthSize
      )
      this.$refs.listResponsePanel.doLayout(
        heightSize * (1 - horizontalDataNum) + 10,
        widthSize
      )
    },
    ...
    handleRun() {
      if (
        !(this.requestApiInfo.status === 1 || this.requestApiInfo.status === 2)
      ) {
        this.$message.error('Api must be Published or Changes.')
        return
      }
      //
      const testResult = checkRequestBody(
        this.requestApiInfo.select,
        this.requestApiInfo.codeType,
        this.requestBody
      )
      if (!testResult) {
        return
      ...
      const self = this
      const requestURL =
        contextPath() + ('/' + this.requestApiInfo.path).replace('//', '/')
      request(
        requestURL,
        {
          direct: true,
