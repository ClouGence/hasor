...
public class Client {
    public static void main(String[] args) throws Throwable {
        RsfContext rsfContext = new RsfBootstrap().doBinder(new RsfStart() {
            public void onBind(RsfBinder rsfBinder) throws UnknownHostException, MalformedURLException {
        ...
        final ITestServices bean = client.getRemote("RSF", "net.hasor.rsf._test.ITestServices", "1.0.0");
        for (int i = 0; i < 20; i++) {
            new Thread() {
                public void run() {
                    call(bean);
    ...
    public static void call(ITestServices bean) {
        for (int i = 0; i < 1000000; i++) {
            try {
                String sayStr = bean.sayHello("你好...(" + i + ")");
                //                System.out.println(sayStr);//发起调用.
            } catch (RsfTimeoutException e) {
                e.printStackTrace();
