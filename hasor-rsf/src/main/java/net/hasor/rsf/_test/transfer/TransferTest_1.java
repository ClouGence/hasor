...
public class TransferTest_1 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        while (true) {
            ...
            final long ops = ((index - n) * 1000) / duration;
    ...
    public static void _1_main(String[] args) throws IOException {
        final TrackManager t = new TrackManager(//
                StationEnum.values(), 20, CAPACITY);
        //
        seller(t, 0).start();
        consumer(t, 0).start();
    }
    private static Thread consumer(final TrackManager track, final int id) {
        return new Thread() {
            public void run() {
                ...
                while (true) {
                    TRead node = track.waitForRead(StationEnum.B);
                    for (int i = 0; i < CAPACITY; i++) {
                        Object data = node.pullGood();
                    }
                    track.switchNext(StationEnum.A);
                }
            }
        };
    }
    private static Thread seller(final TrackManager track, final int id) {
        return new Thread() {
            public void run() {
                ...
                while (true) {
                    TWrite node = track.waitForWrite(StationEnum.A);
                    for (int i = 0; i < CAPACITY; i++) {
                        node.pushGood(index++);
                    }
                    track.switchNext(StationEnum.B);
                }
            }
        };
    }
