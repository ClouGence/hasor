...
class AbstractAuthSession implements AuthSession {
    ...
    protected AbstractAuthSession(String sessionID, AbstractSecurityContext securityContext) {
        Hasor.assertIsLegal(!StringUtils.isBlank(sessionID), "sessionID is Undefined!");
        Hasor.assertIsNotNull(securityContext, "SecurityContext is Undefined!");
    ...
    @Override
    public synchronized void doLogin(String authSystem, Token user) throws SecurityException {
        this.checkClose();/*Check*/
        Hasor.assertIsNotNull(user);
    ...
    public synchronized void doLogout() throws SecurityException {
        ...
        Hasor.debug("%s :doLogout!", this.sessionID);
    };
    @Override
