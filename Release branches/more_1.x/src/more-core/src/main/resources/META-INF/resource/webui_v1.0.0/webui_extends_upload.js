...
WebUI.Component.$extends("ui_Upload", "UIInput", {
    ...
    initUpload : function() {
        ...
        buttonImage = (buttonImage != null) ? buttonImage : WebUI.variable("WebUI_Var_Library") + "/swfupload_v2.2.0.1/upload_65x29.png";
        //
        var sendData = {};
        ...
        //
        var settings = {
            flash_url : WebUI.variable("WebUI_Var_Library") + "/swfupload_v2.2.0.1/falsh/swfupload.swf",// flash版本
            flash9_url : WebUI.variable("WebUI_Var_Library") + "/swfupload_v2.2.0.1/falsh/swfupload_fp9.swf",// 版本
            ...
            file_types_description : this.allowFilesDesc(),// 文件类型描述，例：Web Image Files
...
WebUI.Component.ui_Upload.handlers.swfupload_load_failed_handler = function() {
    // 1.通知用户程序
    if (WebUI.isFun(webui_swfupload_load_failed_handler) == true)
        webui_swfupload_load_failed_handler();
};
/** 发生错误版本。 */
WebUI.Component.ui_Upload.handlers.swfupload_preload_handler = function() {
    // 1.通知用户程序
...
/** 当某个文件被加入到上传队列 */
WebUI.Component.ui_Upload.handlers.file_queued_handler = function(file) {
    // 1.进度条处理
...
/** 上传队列中发生错误 */
WebUI.Component.ui_Upload.handlers.file_queue_error_handler = function(file, errorCode, message) {
    // 1.进度条处理
    if (this.customSettings.componentObject.showProgress() == true) {
        var progress = new WebUI_FileProgress(file, this.customSettings.componentObject.progressTargetID());
        progress.setError();
...
/** 上传对话框关闭之后。 */
WebUI.Component.ui_Upload.handlers.file_dialog_complete_handler = function(numFilesSelected, numFilesQueued) {
    // 1.启动上传
...
/** 当某个文件开始上传 */
WebUI.Component.ui_Upload.handlers.upload_start_handler = function(file) {
    // 1.进度条处理
    if (this.customSettings.componentObject.showProgress() == true) {
        var progress = new WebUI_FileProgress(file, this.customSettings.componentObject.progressTargetID());
...
/** 文件上传过程中调用，用于更新进度。 */
WebUI.Component.ui_Upload.handlers.upload_progress_handler = function(file, bytesLoaded, bytesTotal) {
    // 1.进度条处理
    if (this.customSettings.componentObject.showProgress() == true) {
...
/** 上传组建级的错误。 */
WebUI.Component.ui_Upload.handlers.upload_error_handler = function(file, errorCode, message) {
    // 1.进度条处理
    if (this.customSettings.componentObject.showProgress() == true) {
        var progress = new WebUI_FileProgress(file, this.customSettings.componentObject.progressTargetID());
        progress.setError();
...
/** 当队列中正在上传的文件完成上传之后调用。 */
WebUI.Component.ui_Upload.handlers.upload_success_handler = function(file, serverData) {
    // 1.进度条处理
    if (this.customSettings.componentObject.showProgress() == true) {
        var progress = new WebUI_FileProgress(file, this.customSettings.componentObject.progressTargetID());
...
/** 每个文件上传完毕都会调用一遍 */
WebUI.Component.ui_Upload.handlers.upload_complete_handler = function(file) {
    // 1.通知用户程序
    if (WebUI.isFun(webui_upload_complete_handler) == true)
        webui_upload_complete_handler(file);
};
/** 当队列上传完毕之后 */
WebUI.Component.ui_Upload.handlers.queue_complete_handler = function(numFilesUploaded) {
    // 1.获取到所有服务器返回的值
