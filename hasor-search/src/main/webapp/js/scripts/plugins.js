...
var render_plugin_data = function(plugin_data, plugin_sort, types) {
	...
	for (var i = 0; i < types.length; i++) {
		...
		navigation_markup += ('<a href="#/' + type_url + '" rel="' + types[i].esc() + '">' + types[i].esc());
		if (changes.count[types[i]]) {
			navigation_markup += ' <span>' + changes.count[types[i]].esc() + '</span>';
		}
		navigation_markup += '</a></li>';
	...
	$('.PLUGINCHANGES a', navigation_element).die('click').live('click', function(event) {
		load_reference_xml();
	...
	$('#recording button').die('click').live('click', function(event) {
		$.ajax({
			type : 'POST',
	...
	$('a[href="' + context_path + '"]', navigation_element).parent().addClass('current');
	...
	$('a', frame_element).off('click').on('click', function(event) {
		var self = $(this);
		...
		url += match ? entry.replace(regex, '') : entry + ',' + bean;
	...
	$('.entry', frame_element).each(function(i, entry) {
		$('.detail > li', entry).not('.stats').filter(':even').addClass('odd');

		$('.stats li:odd', entry).addClass('odd');
	});
};
...
sammy.bind('plugins_load', function(event, params) {
	...
	$.ajax({
		...
		success : function(response, text_status, xhr) {
			...
			$.get('tpl/plugins.html', function(template) {
...
// #/:core/plugins/$type
sammy.get(new RegExp(app.core_regex_base + '\\/(plugins)\\/(\\w+)$'), function(context) {
...
// #/:core/plugins
sammy.get(new RegExp(app.core_regex_base + '\\/(plugins)$'), function(context) {
	core_basepath = this.active_core.attr('data-basepath');
